...
Perl backwards compatible alternative to `set-font-size`. Font metrics are scaled for individual glyphs, but are not scaled for font metrics. The `scaled-metrics` method may be called to get the scaled metrics.
